...
public class pageView extends View {
    ...
    ColorMatrixColorFilter mColorMatrixFilter;  //翻起页背面的颜色绘制
    Matrix mMatrix;
    ...
    private void drawCurrentPageArea(Canvas canvas, Bitmap bitmap, Path path) {
        mPath0.reset();
        mPath0.moveTo(mBezierStart1.x, mBezierStart1.y);  //设置起点坐标
        mPath0.quadTo(mBezierControl1.x, mBezierControl1.y, mBezierEnd1.x,
                mBezierEnd1.y);//添加一条2次贝塞尔曲线
        mPath0.lineTo(mTouch.x, mTouch.y);  //从上一最后点添加一条直线到指定的点
        ...
        canvas.save();
        canvas.clipPath(path, Region.Op.XOR);//XOR补集 就是全集的减去交集余下部分显示
    ...
    private void drawNextPageAreaAndShadow(Canvas canvas, Bitmap bitmap) {
        ...
        canvas.save();
        try {
            canvas.clipPath(mPath0);//与指定路径相交的当前剪辑
            canvas.clipPath(mPath1, Region.Op.INTERSECT);//INTERSECT交集显示
        } catch (Exception e) {
        }
    ...
    private void startAnimation(int delayMillis) {
        ...
        if (mCornerY > 0) {
            ...
            dy = (int) (mScreenHeight - mTouch.y)-1;
        } else {
